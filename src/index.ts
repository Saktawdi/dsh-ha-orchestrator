...
import type { Config, HaConfig } from './config.js'
import {
  createHaState,
  keyOf as haKeyOf,
...
import type { Initializer } from './remote.js'
...
interface StateSnapshot {
  config: Config
  quarantine: Array<{ provider: string; model: string; code?: string; remainingMs: number }>
...
interface RunRecord {
  ...
  durationMs?: number
  aborted: boolean
  /** 本 run 由哪个 runId 恢复而来（resume 语义）。 */
  resumedFrom?: string
...
/**
 * 面向客户端列表/悬浮面板的轻量 run 记录。
 *
 * 完整 RunRecord 会携带每个任务的 prompt/output；并发子智能体较多时，周期性把
 * 最近几十条完整记录传到浏览器会产生明显的 JSON 编解码、GC 与 React 保留成本。
 * 轻量视图只保留列表和会话定位实际使用的字段。
 */
...
async function apply(ctx: Context): Promise<void> {
  ...
  const state: {
    config: Config
    quarantine: HaState['quarantine']
    failures: HaState['failures']
  ...
  let langDicts: Partial<Record<TargetLang, Dict | null>> = { zh: null, en: null }
  ...
  async function readPluginFile(rel: string): Promise<string | null> {
    ...
    try {
      const fsService = getService<FsService>(ctx, 'fs')
  ...
  async function loadDictionaries(): Promise<Partial<Record<TargetLang, Dict | null>>> {
  ...
  function dshLocaleNow(): string | null {
    try {
      const settings = getService<SettingsService>(ctx, 'settings')
  ...
  async function applyLanguage(): Promise<LangState> {
  ...
  function maybeRefreshLanguage(): Promise<unknown> {
  ...
  function i18nSnapshot(): StateSnapshot['i18n'] {
  ...
  function debugLog(level: string, ev: string, msg: string, data?: unknown): void {
    ...
    let safe: unknown = undefined
    if (data !== undefined) {
      try { safe = JSON.parse(JSON.stringify(data)) } catch (e) { safe = String(data) }
    }
    const entry: DebugLogEntry = { at: new Date().toISOString(), level, ev, msg: String(msg) }
  ...
  function keyOf(provider: string, model?: string | null): string { return haKeyOf(provider, model) }
  function splitKey(k: string): [string, string] { return haSplitKey(k) }
  function matchesCodes(codes: string[] | null | undefined, code: string): boolean { return haMatchesCodes(codes, code) }
  function clearExpired(): void { haClearExpired(state, now()) }
  ...
  // 磁盘只保留最近 RUN_FILE_CAP 条，完整输出内存只保留最近 RUN_MEM_CAP 条。
  ...
  function summarizeRunRecord(rec: RunRecord): RunRecordSummary {
    const runs = Array.isArray(rec.runs) ? rec.runs : []
    return {
      runId: String(rec.runId || ''),
      mode: resolveMode(String(rec.mode || 'fanout')),
      agent: String(rec.agent || ''),
      sessionId: String(rec.agent || ''),
      startedAt: String(rec.startedAt || ''),
      ...(rec.finishedAt ? { finishedAt: String(rec.finishedAt) } : {}),
      ...(rec.durationMs !== undefined ? { durationMs: Math.max(0, Number(rec.durationMs) || 0) } : {}),
      aborted: !!rec.aborted,
      ...(rec.resumedFrom ? { resumedFrom: String(rec.resumedFrom) } : {}),
      // 诊断展开只显示前 2000 字；在 host 侧截断，避免先跨 RPC 传完整 summary。
      ...
      runs: runs.map((run) => ({
        ...
        ...(run.lastKey ? { lastKey: String(run.lastKey) } : {}),
  ...
  async function readRunSummariesFromDisk(): Promise<RunRecordSummary[]> {
    ...
    try {
      ...
      for (const line of String(text).split(/\r?\n/)) {
        ...
        try {
          const rec = JSON.parse(line) as RunRecord
          if (rec && rec.runId) out.push(summarizeRunRecord(rec))
        } catch (e) { /* 跳过损坏行 */ }
      }
      runSummaryCache = out.sort((a, b) => {
        const at = String(a.startedAt)
        const bt = String(b.startedAt)
  ...
  async function recentRunSummaries(limit: number, sessionIds: string[]): Promise<RunRecordSummary[]> {
    const disk = await readRunSummariesFromDisk()
    const byId = new Map<string, RunRecordSummary>()
    for (const rec of state.runs) byId.set(rec.runId, summarizeRunRecord(rec))
    for (const rec of disk) if (!byId.has(rec.runId)) byId.set(rec.runId, rec)
    const allowed = sessionIds.length > 0 ? new Set(sessionIds) : null
    return [...byId.values()]
      .filter((rec) => !allowed || allowed.has(rec.sessionId))
      .sort((a, b) => {
        const at = String(a.startedAt)
        const bt = String(b.startedAt)
        return at < bt ? 1 : (at > bt ? -1 : 0)
  ...
  function recordRun(rec: RunRecord): void {
    ...
    if (runSummaryCache) {
      runSummaryCache = [summarizeRunRecord(rec)].concat(runSummaryCache.filter((item) => item.runId !== rec.runId)).slice(0, RUN_FILE_CAP)
    }
    persistRun(rec)
  ...
  async function acquireOrchSlot(): Promise<void> {
    ...
    await new Promise<void>((resolve) => { orchWaiters.push(resolve) })
    activeOrchSlots += 1
  }
  function releaseOrchSlot(): void {
    if (activeOrchSlots > 0) activeOrchSlots -= 1
  ...
  function registeredProviders(): Set<string> {
    const tNow = now()
    if (providerCache && tNow - providerCacheAt < 30000) return providerCache
    const llm = getService<LlmService>(ctx, 'llm')
    let set: Set<string> | null = null
    try {
      if (llm) {
        set = new Set(llm.listProviders().map((p) => String((p && (p as { id?: string; provider?: string; name?: string }).id) || (p as { provider?: string }).provider || (p as { name?: string }).name || p)))
      }
    } catch (e) { set = null }
  ...
  function findFallback(agentId: string, excludeKey: string | null): FallbackCandidate | null {
    return haFindFallback(state, state.config.ha, registeredProviders(), agentId, excludeKey, now())
  }
  function pickFallback(agentId: string, excludeKey: string | null): { provider: string; model: string; reasoningEffort?: string; key: string } | null {
    return haPickFallback(state, state.config.ha, registeredProviders(), agentId, excludeKey, now())
  }
  function hasFallback(agentId: string, excludeKey: string | null): boolean {
  ...
  async function backoff(retries: number): Promise<void> {
    const timer = getService<TimerService>(ctx, 'timer')
  ...
  function tryPersist(target: { provider: string; model: string }): void {
    const adm = getService<AgentDefaultModelService>(ctx, 'agentDefaultModel')
    if (!adm) return
    try { adm.saveSelection({ provider: target.provider, model: target.model }) } catch (e) { console.error('saveSelection failed', e) }
  }
  function currentDefaultSelection(): { provider?: string; model?: string } | null {
    const adm = getService<AgentDefaultModelService>(ctx, 'agentDefaultModel')
    if (!adm) return null
  ...
  function fsServiceNow(): FsService | null {
    try { return getService<FsService>(ctx, 'fs') } catch (e) { return null }
  }
  ...
  function storageDir(): string {
    ...
    try {
      const env = getService<{ get(name: string): { value?: string } | undefined }>(ctx, 'launchEnvironment')
  ...
  function sandboxWritableRoot(): string {
    try {
      const sp = getService<SandboxPolicyService>(ctx, 'sandboxPolicy')
  ...
  async function resolveStorageTargetIn(dir: string | null, name: string): Promise<{ displayPath?: string } | null | undefined> {
    const fsService = fsServiceNow()
  ...
  async function readStorageTextIn(dir: string | null, name: string): Promise<string | null> {
    const fsService = fsServiceNow()
    const target = await resolveStorageTargetIn(dir, name)
  ...
  async function readStorageText(name: string): Promise<string | null> {
  ...
  function parseConfigJson(text: string | null | undefined): Record<string, unknown> | null {
  ...
  let configLoadPromise: Promise<unknown> | null = null
  let configLoadRetries = 0
  const CONFIG_LOAD_MAX_RETRIES = 30
  const CONFIG_LOAD_RETRY_MS = 2000
  async function retryLoadPersistedConfig(): Promise<void> {
  ...
  function scheduleConfigLoadRetry(): void {
    if (configLoadRetries >= CONFIG_LOAD_MAX_RETRIES) return
    configLoadRetries += 1
    const timer = getService<TimerService>(ctx, 'timer')
    if (!timer || typeof timer.timeout !== 'function') return
    try { timer.timeout(() => { void retryLoadPersistedConfig() }, CONFIG_LOAD_RETRY_MS) } catch (e) { /* ignore */ }
  }
  // 供 stateGet 调用：如果启动时没加载成功，则在这里补一次加载；成功后重建工具并跟随语言。
  function ensureConfigLoaded(): Promise<unknown> {
  ...
  let persistState: PersistState = { ok: false, path: '', error: '' }
  // 失败时的诊断信息：fs 服务类型、writeText 是否存在、沙箱策略
  function persistDiag(): Record<string, unknown> {
    ...
    try {
      ...
      if (sp && typeof sp.resolve === 'function') {
        ...
        } catch (e) { out.policyError = String((e && (e as Error).message) || e) }
      }
    } catch (e) { out.error = String((e && (e as Error).message) || e) }
    return out
  }
  async function persistConfig(): Promise<boolean> {
    ...
    for (const dir of dirs) {
      ...
      try {
        ...
        persistState = { ok: true, path: String((target as { displayPath?: string }).displayPath || ''), error: '' }
        console.log('[ha] config persisted:', persistState.path)
        debugLog('info', 'config.persisted', '配置已写入磁盘', { file: persistState.path })
        return true
      } catch (e) {
        lastErr = String((e && (e as Error).message) || e)
      }
    }
  ...
  ctx.on('agent/request', async (payload: { agent: Agent; turn: number; step: number; signal: AbortSignal }, next: () => Promise<LlmCallConfig>): Promise<LlmCallConfig> => {
    try {
      ...
      const out = { ...rest, provider: target.provider, model: target.model } as LlmCallConfig
      if (target.reasoningEffort) (out as unknown as Record<string, unknown>).reasoningEffort = target.reasoningEffort
      return out
    } catch (e) {
  ...
  ctx.on('agent/error', (payload: { agent: Agent; turn: number; step: number; error: unknown }): void => {
    try {
      const cfg: HaConfig = state.config.ha
  ...
  function buildOrchestrateTool(): ReturnType<typeof defineTool> {
    ...
    return defineTool({
      ...
      async execute(args, exec) {
        ...
        let attemptedTasks: Array<{ id: string; label: string; agent: string; prompt: string }> = []
        // resume 恢复出的原记录（失败留痕时复用其完整任务定义）。
        // 该变量在 try 内被直接赋值，catch 读取时不会被 TS 窄化为 never。
        let resumePrevRecForCatch: RunRecord | null = null
        try {
          ...
          let presetSupervisorAgent: string | undefined
          let presetMergeInstructions: string | undefined
          let presetTasks: Array<{ id?: string; label?: string; agent?: string; prompt: string }> | undefined
          if (args.preset) {
            const preset = (cfg.presets || []).find((p) => p && p.name === String(args.preset))
            if (!preset) throw new Error(t('orch.errNoPreset', { name: String(args.preset) }))
          ...
          const mode: OrchestrateMode = args.mode ? resolveMode(args.mode) : (presetMode || 'fanout')
          ...
          const makeErrorRun = (task: TaskLike, e: unknown): RunResultLike => {
          ...
          let resumeCompleted: RunResultLike[] = []
          let resumeCarry = ''
          let runTasks: TaskLike[] = tasks
          ...
          if (args.resume) {
            ...
            if (!prevRec) throw new Error(t('orch.errNoRun', { runId: String(args.resume) }))
          ...
          if (resumePrevRec) {
            ...
            for (const r of resumeCompleted) byId.set(r.id, r)
            for (const r of finalRuns) byId.set(r.id, r)
            const ordered: RunResultLike[] = []
            for (const tk of resumePrevRec.tasks) {
              const run = byId.get(tk.id || tk.label || 'task')
              if (run && !ordered.some((x) => x.id === run.id)) ordered.push(run)
            }
            for (const r of byId.values()) if (!ordered.some((x) => x.id === r.id)) ordered.push(r)
          ...
          const partialRuns = (e as Error & { partialRuns?: RunResultLike[] }).partialRuns
          ...
          const failedTasks = resumePrevRecForCatch && Array.isArray(resumePrevRecForCatch.tasks) && resumePrevRecForCatch.tasks.length > 0
            ...
            : (attemptedTasks.length > 0
                ...
                : Array.isArray(args.tasks)
                  ? args.tasks.map((tk: { id?: string; label?: string; agent?: string; prompt?: string }) => ({ id: tk.id || '', label: tk.label || '', agent: tk.agent || '', prompt: tk.prompt || '' }))
                  : [])
          ...
          if (completedLeaf.length > 0 && completedLeaf.length < failedTasks.length) {
            ...
            const wrapped = new Error(String((e && (e as Error).message) || e) + hint)
            if (e && typeof e === 'object') {
              for (const key of Object.keys(e)) {
                try { (wrapped as unknown as Record<string, unknown>)[key] = (e as unknown as Record<string, unknown>)[key] } catch { /* 复制失败不影响主流程 */ }
              }
            }
            throw wrapped
          }
  ...
  let contextInjectDispose: (() => void) | null = null
  let contextInjectRetries = 0
  const injectionStatus: InjectionStatus = { registered: false, order: 40, reason: '', lastEval: null }
  function injectionStatusSnapshot(): InjectionStatus {
    return {
  ...
  function installContextInjection(): void {
    try { if (contextInjectDispose) { contextInjectDispose(); contextInjectDispose = null } } catch (e) { /* ignore */ }
    const sp = getService<SystemPromptService>(ctx, 'systemPrompt')
    ...
    try {
      ...
      debugLog('error', 'ctx.inject.install.failed', '上下文注入段落注册失败', { error: String((e && (e as Error).message) || e) })
    }
  }
  function scheduleContextInjectionRetry(): void {
  ...
  // settings/updated 事件名不在 cordis 核心 Events 声明内（dsh-settings 类型未入图），
  // 载荷为命名空间字符串；经窄化签名挂接，运行时行为不变。
  ;(ctx.on as unknown as (name: string, listener: (ns: string) => void) => unknown)('settings/updated', (ns: string): void => {
    if (ns !== 'locale') return
    const mode = state.config.lang && state.config.lang.mode
    ...
    try {
      ...
      if (llm) {
        llmProviders = llm.listProviders().map((p) => ({
          provider: String((p && (p as { id?: string }).id) || (p as { provider?: string }).provider || (p as { name?: string }).name || p),
          name: String((p && (p as { name?: string }).name) || (p as { id?: string }).id || (p as { provider?: string }).provider || p),
        }))
      }
    } catch (e) { llmProviders = [] }
    return llmProviders
  }
  function subagentList(): string[] {
    const subagents = getService<SubagentProvider>(ctx, 'subagents')
  ...
  function parseAgentJson(text: string): Record<string, unknown> | null {
    ...
    try { return JSON.parse(body) as Record<string, unknown> } catch (e) { return null }
  }
  ...
  async function generateAgent(requirementRaw: unknown): Promise<{ agent: { name: string; provider: string; model: string; description: string; systemPrompt: string } }> {
    ...
    if (!subagents) throw new Error(t('orch.errNoService'))
    const provider = resolveProvider()
    const agentsSvc = getService<AgentsService>(ctx, 'agents')
    const parent: AgentLike | null = (agentsSvc && ((agentsSvc.currentInitiator ? agentsSvc.currentInitiator() : null) || agentsSvc.list()[0])) || null
    ...
    try {
      const llm = getService<LlmService>(ctx, 'llm')
      if (llm) providers = llm.listProviders().map((p) => String((p && (p as { id?: string }).id) || (p as { provider?: string }).provider || (p as { name?: string }).name || p))
    } catch (e) { providers = [] }
    ...
    const task: TaskLike = { id: 'gen-agent', label: 'generate', prompt }
    const def: AgentDefLike | null = sel && sel.provider && sel.model ? { name: '', provider: sel.provider, model: sel.model } : null
    ...
    let run: RunResultLike
    if (typeof parent.runMaintenance !== 'function') {
      throw new Error('运行时不支持 runMaintenance，无法生成子智能体')
    }
    try {
      run = await parent.runMaintenance((signal: AbortSignal) => runOne(subagents, provider, task, '', parent, signal, def))
    } catch (e) {
      const busyMsg = String((e && (e as Error).message) || e)
    ...
    return {
      agent: {
        name: uniqueAgentName(String((parsed as { name?: unknown }).name || '')),
        provider: String((parsed as { provider?: unknown }).provider || ''),
        model: String((parsed as { model?: unknown }).model || ''),
        description: String((parsed as { description?: unknown }).description || ''),
        systemPrompt: String((parsed as { systemPrompt?: unknown }).systemPrompt || ''),
      },
    }
  }

  // ---- Remote RPC 服务（client: ctx.remote.haOrchestrator.<method>） ----
  const remoteInitializers: Initializer[] = []
  class HaOrchestratorRpc extends TypertRemoteService {
    constructor() {
      super(ctx, 'haOrchestrator')
      runInitializers(this, remoteInitializers)
    }
    async stateGet(): Promise<StateSnapshot & Record<string, unknown>> {
    ...
    async stateReload(): Promise<StateSnapshot & Record<string, unknown>> {
      ...
      if (JSON.stringify((state.config.orch || {}).agents || []) !== agentsBefore) {
        try { reinstallTools() } catch (e) { console.error('[ha] stateReload reinstall tools failed', e) }
      }
    ...
    async stateSet(args: { patch?: unknown }): Promise<StateSnapshot & Record<string, unknown>> {
      ...
      debugLog('info', 'rpc.stateSet', '配置已更新', {
        ...
        debug: patch && (patch as { debug?: unknown }).debug ? (patch as { debug: unknown }).debug : undefined,
        lang: patch && (patch as { lang?: unknown }).lang ? (patch as { lang: unknown }).lang : undefined,
        ctx: patch && (patch as { ctx?: unknown }).ctx ? (patch as { ctx: unknown }).ctx : undefined,
      })
      if (patch && (patch as { ctx?: unknown }).ctx && typeof (patch as { ctx: unknown }).ctx === 'object') {
    ...
    async stateImport(args: { json?: string }): Promise<StateSnapshot & Record<string, unknown>> {
      ...
      if (Object.keys(patch).length === 0) throw new Error(t('sys.importInvalid'))
      // 整体替换：先以默认配置为底，再把导入节覆盖上去，缺失节回退默认
      const next = { ...JSON.parse(JSON.stringify(defaultConfig)) as Config, ...patch }
      await applyConfigNext(next)
    ...
    async modelsList(args: { provider?: string }): Promise<Array<{ provider: string; model: string; name: string }>> {
      ...
      try {
        ...
        return (infos || []).map((m) => ({
          provider: String((m as { provider?: string }).provider || provider),
          model: String((m as { id?: string }).id || (m as { model?: string }).model || (m as { name?: string }).name || m),
          name: String((m as { name?: string }).name || (m as { id?: string }).id || (m as { model?: string }).model || m),
        }))
      } catch (e) { return [] }
    }
    agentsGenerate(args: { requirement?: unknown }): Promise<{ agent: { name: string; provider: string; model: string; description: string; systemPrompt: string } }> {
    ...
    haReset(): StateSnapshot & Record<string, unknown> {
      state.quarantine.clear()
      state.failures.clear()
      state.perAgent.clear()
      state.history = []
    ...
    async orchRuns(): Promise<{ runs: RunRecord[] }> {
      const all = await mergedRunRecords()
      return jsonSafe({ runs: all.slice(0, RUN_RPC_CAP) }, { runs: [] })
    }
    // 客户端轮询专用轻量历史：不返回 prompt/output，并支持按会话血缘在 host 侧过滤。
    async orchRecent(args: { limit?: unknown; sessionIds?: unknown } = {}): Promise<{ runs: RunRecordSummary[] }> {
      const limit = Math.max(1, Math.min(RUN_RPC_CAP, Number(args && args.limit) || 24))
      const rawSessionIds = args && args.sessionIds
      const sessionIds = Array.isArray(rawSessionIds)
        ? rawSessionIds.map((id: unknown) => String(id || '')).filter(Boolean).slice(0, 64)
        : []
      const runs = await recentRunSummaries(limit, sessionIds)
      return jsonSafe({ runs }, { runs: [] })
    }
    ...
    async orchSavePreset(args: { name?: string; mode?: string; tasks?: Array<{ id?: string; label?: string; agent?: string; prompt: string }>; agent?: string; supervisorAgent?: string; mergeInstructions?: string }) {
      ...
      if (tasks.length === 0) throw new Error(t('orch.errNoTasks'))
      const entry = {
        name,
    ...
    debugClear(): { enabled: boolean; logs: DebugLogEntry[] } {
      state.debugLogs = []
  ...
  decorateRemoteMethod(Remote, HaOrchestratorRpc, 'orchRecent', 'orchRecent', remoteInitializers)
  ...
  const REMOTE_METHODS: Array<{ method: string; args: boolean }> = [
  ...
  function installTypertRemoteDescriptors(): void {
    const typert = getService<TypertRegistryService>(ctx, 'typert')
    ...
    try {
      ...
      debugLog('error', 'rpc.typert.register.failed', 'Typert Remote 描述符注册失败', { error: String((e && (e as Error).message) || e) })
    }
  }
