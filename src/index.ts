      ...
      async execute(args, exec) {
        ...
        try {
          ...
          if (args.resume) {
            ...
            if (runTasks.length === 0) throw new Error(t('orch.errRunDone', { runId: String(args.resume) }))
          ...
          const failedRec: RunRecord = {
            ...
            tasks: Array.isArray(args.tasks) ? args.tasks.map((tk: { id?: string; label?: string; agent?: string; prompt?: string }) => ({ id: tk.id || '', label: tk.label || '', agent: tk.agent || '', prompt: tk.prompt || '' })) : [],
            runs: normalizeFinalRuns(runs),
