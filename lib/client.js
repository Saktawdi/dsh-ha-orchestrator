...
`
    ...
    function apply(ctx) {
        ...
        React.useEffect(() => {
          ...
          try {
            ...
            const schedule = () => {
              ...
              // 流式输出可能每个 token 都触发 session 更新；token 统计最多每秒重算一次。
              pending = setTimeout(commit, 900)
            }
            commit()
      ...
      function SubagentOverlay() {
        ...
        const refreshRuns = (scope) => rpc.orchRecent({ limit: 24, sessionIds: Array.isArray(scope) ? scope : sessionScope }).then((res) => {
          if (disposedRef.current || !openRef.current) return
        ...
        React.useEffect(() => {
          disposedRef.current = false
          refreshActive()
          // 面板关闭时只轮询很小的 active 视图，用于 FAB 状态；不再拉取配置和历史。
          const activeDispose = installInterval(refreshActive, open ? 2200 : 4500)
          return () => {
        ...
        React.useEffect(() => {
          ...
          refreshCatalogs()
          const stateDispose = installInterval(refreshState, 30000)
          const runsDispose = installInterval(() => refreshRuns(sessionScope), 12000)
          return () => { stateDispose(); runsDispose() }
