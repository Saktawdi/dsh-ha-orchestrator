...
`
    ...
    function apply(ctx) {
      ...
      function SubagentUsage(props) {
        ...
        }, [initialUsage && initialUsage.total])
      ...
      function SubagentOverlay() {
        ...
        const refreshRuns = (scope) => {
          const promise = openRef.current ? rpc.orchRuns() : rpc.orchRecent({ limit: 24, sessionIds: Array.isArray(scope) ? scope : sessionScope })
      ...
      // 所有 slots.inject 的 dispose 句柄统一入列：ctx.effect 可用时挂接终止清理，
      // 避免插件热更新/卸载后残留注册（zombie UI）；重注册路径自管的不受影响。
      let sectionDispose = null
      const slotDisposes = []
      const trackSlotDispose = (dispose) => {
        if (typeof dispose === 'function') slotDisposes.push(dispose)
        return dispose
      }
      function registerSection() {
        try { if (sectionDispose) sectionDispose() } catch (e) { /* ignore */ }
        sectionDispose = trackSlotDispose(slots.inject('settings.section', () => slots.register(
      ...
      registerSection()
      trackSlotDispose(slots.inject('tool.view.cordis', () => slots.register(
        { name: 'tool.view.cordis', key: 'self' },
        () => React.createElement(HaStatusCard),
      )))
      // 对话内 Run 卡片：orchestrate 调用的 keyed toolview（key 域开放，任意工具名）
      trackSlotDispose(slots.inject('tool.call.toolview', () => slots.register(
      ...
      trackSlotDispose(slots.inject('shell.overlay', () => slots.register(
        { name: 'shell.overlay', id: 'dsh-ha-orchestrator-subagents', order: 32 },
        () => React.createElement(SubagentOverlay),
      )))
      // 终止清理：客户端运行时提供 ctx.effect（Cordis fiber 生命周期）时挂接；
      // dispose 可能被框架再次调用（幂等/抛错均吞掉），双清无害。
      try {
        if (ctx && typeof ctx.effect === 'function') {
          ctx.effect(() => () => {
            for (let i = 0; i < slotDisposes.length; i += 1) {
              try { slotDisposes[i]() } catch (e) { /* ignore */ }
            }
            slotDisposes.length = 0
            sectionDispose = null
          })
        }
      } catch (e) { /* 旧 runtime 无 effect 时保持原有行为 */ }
    }
