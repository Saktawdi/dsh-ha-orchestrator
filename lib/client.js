    ...
    function apply(ctx) {
      ...
      function DiagnosticsCard(props) {
        ...
        return React.createElement(Card, { icon: 'pulse', title: t('diag.title'), subtitle: t('diag.subtitle'), collapsible: true, defaultCollapsed: true },
          ...
          React.createElement(RunHistoryList, { runs, limit: 8 }),
          React.createElement(Row, { label: '' },
      ...
      function subagentCatalogItems(snapshot, state, activeRuns, runs, parentId) {
        ...
        const items = entries.filter((entry) => entry && entry.kind === 'child').map((entry) => {
          ...
          const explicitStatus = (match.activeTask && match.activeTask.status) || (match.result && match.result.status) || sessionStatus
          const status = explicitStatus || (entry.activity === 'running' || (summary && summary.running) ? 'running' : 'completed')
      ...
      function subagentItems(activeRuns, runs, state) {
        ...
        return (Array.isArray(latest.runs) ? latest.runs : []).map((result, index) => {
          ...
          return {
            ...
            tokensAgentId: String((result && result.agentId) || ''),
            agentId: String((result && result.agentId) || ''),
      ...
      function SubagentUsage(props) {
        ...
        const scopeKey = String(props.scopeKey || '')
        const [value, setValue] = React.useState(agentId ? null : { missing: true })
        React.useEffect(() => {
          let disposed = false
          if (!agentId) {
            setValue({ missing: true })
            return undefined
          }
          try {
            const sessions = ctx.get('sessions')
            ...
            const read = () => {
              if (!disposed) setValue({ usage: readSubagentUsage(session) })
            }
            read()
            const unsubscribe = typeof session.subscribe === 'function' ? session.subscribe(read) : null
            const snapshot = typeof session.getSnapshot === 'function' ? session.getSnapshot() : null
            if (snapshot && snapshot.openState === 'cold' && typeof session.open === 'function') {
              session.open().then(read).catch(() => {})
            }
            return () => {
              disposed = true
            ...
            if (!disposed) setValue({ missing: true })
            return undefined
          }
        }, [agentId, scopeKey])
        ...
        return React.createElement('div', { className: 'hao-subagent-row hao-subagent-row-' + kind + (canDrill ? ' hao-subagent-row-parent' : ''), role: 'group', onClick: openSession },
          React.createElement('button', { type: 'button', className: 'hao-subagent-row-main', title: t('subagent.openSession'), onClick: (event) => { event.stopPropagation(); openSession() } },
            ...
            React.createElement('span', { className: 'hao-subagent-row-meta' },
              ...
              React.createElement('span', { className: 'hao-subagent-row-tokens' }, React.createElement(Icon, { name: 'pulse', size: 11 }), React.createElement(SubagentUsage, { agentId: item.tokensAgentId, scopeKey: props.scopeKey })),
            ),
          ),
      ...
      function SubagentOverlay() {
        const [open, setOpen] = React.useState(false)
        const [state, setState] = React.useState(null)
        const [activeRuns, setActiveRuns] = React.useState(null)
        const [runs, setRuns] = React.useState(null)
        const [error, setError] = React.useState('')
        const disposedRef = React.useRef(false)
        const [sessionSnapshot, setSessionSnapshot] = React.useState(readOverlaySessionSnapshot)
        ...
        const rootCatalogId = overlaySessionRoot(sessionSnapshot)
        ...
        const refreshRuns = () => rpc.orchRuns().then((res) => {
          if (disposedRef.current) return
        ...
        const refreshAll = () => { refreshState(); refreshActive(); refreshRuns(); refreshCatalogs() }
        React.useEffect(() => {
          disposedRef.current = false
          refreshAll()
          const stateDispose = installInterval(refreshState, 10000)
          const activeDispose = installInterval(refreshActive, 1800)
          const runsDispose = installInterval(refreshRuns, 5000)
          return () => {
            disposedRef.current = true
            stateDispose(); activeDispose(); runsDispose()
          }
        }, [])
        React.useEffect(() => {
          ...
          sync()
          bind()
          const scopeDispose = installInterval(() => { sync(); bind() }, 1000)
          return () => {
            scopeDispose()
            if (typeof unsubscribe === 'function') unsubscribe()
          }
        ...
        React.useEffect(() => {
          ...
          if (rootCatalogId) desired.add(rootCatalogId)
        ...
        const scopedRuns = Array.isArray(runs) ? runs.filter((run) => overlaySessionMatches(run, sessionScope)) : []
        const catalogView = subagentCatalogItems(sessionSnapshot, state, scopedActiveRuns, scopedRuns, currentCatalogParentId)
        const isDrillPending = currentCatalogParentId !== rootCatalogId && !catalogView.available
        const hasCatalogSnapshot = !!(sessionSnapshot && sessionSnapshot.subagentsByParent && typeof sessionSnapshot.subagentsByParent === 'object')
        const catalogPending = !!rootCatalogId && hasCatalogSnapshot && !catalogView.available
        // 宿主已提供目录快照时，目录是唯一的数据源；等待目录期间不能
        // 回退到 run 记录，否则会把根会话和后代任务平铺到同一层。
        const items = catalogView.available ? catalogView.items : (hasCatalogSnapshot ? [] : subagentItems(scopedActiveRuns, scopedRuns, state))
        ...
        const latest = scopedRuns.length > 0 ? scopedRuns[0] : null
        ...
        return React.createElement('div', { className: 'hao-subagent-anchor' },
          open ? React.createElement('div', { className: 'hao-subagent-panel', role: 'dialog', 'aria-label': t('subagent.title') },
            ...
            activeRuns === null || runs === null || catalogPending || (catalogView.available && catalogView.loading && items.length === 0) || isDrillPending
