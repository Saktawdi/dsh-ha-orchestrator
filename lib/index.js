...
// Emulate the standard-decorator marker the `Remote` decorator records:
// `Remote(export)` returns a decorator that only uses `context.addInitializer`,
// whose callback runs with `this` = an instance (so the marker lands on the
// class prototype). We supply a fake instance whose prototype is the class
// prototype and run the initializer immediately.
function remoteDecorate(proto, methodName, exportName) {
  Remote(exportName)(proto[methodName], {
    ...
    addInitializer(fn) { fn.call(Object.create(proto)) },
  })
}
...
async function apply(ctx) {
  ...
  // ================= 语言系统 =================
  // 语言包位于插件包根目录 `.language/`（zh.ts / en.ts），键集以 zh.ts 为基准。
  ...
  async function loadDictionaries() {
    const zh = parseDictModule(await readPluginFile('.language/zh.ts'))
    const en = parseDictModule(await readPluginFile('.language/en.ts'))
    langDicts = { zh, en }
    return langDicts
  }
  ...
  // 精确键隔离：用于挑选备用（通配隔离不应连坐同 provider 的其它模型）
  function isExactQuarantined(provider, model) {
    clearExpired()
    return state.quarantine.has(keyOf(provider, model))
  }
  // 请求隔离判定：精确键命中，或 model 未知时降级写的 provider 通配键命中
  function isBlocked(provider, model) {
    clearExpired()
    return state.quarantine.has(keyOf(provider, model)) || state.quarantine.has(keyOf(provider, '*'))
  }
  function entryFor(agentId) { return state.perAgent.get(agentId) || { index: 0 } }
  function setEntry(agentId, patch) { state.perAgent.set(agentId, { ...entryFor(agentId), ...patch }) }
  ...
  function findFallback(agentId, excludeKey) {
    ...
    for (let i = 0; i < n; i += 1) {
      ...
      if (isExactQuarantined(b.provider, b.model)) continue
      return { provider: String(b.provider), model: String(b.model), reasoningEffort: b.reasoningEffort || undefined, key: k, index: (idx + 1) % n }
    }
    return null
  }
  // 真正挑选：推进游标（唯一副作用入口）
  function pickFallback(agentId, excludeKey) {
    const found = findFallback(agentId, excludeKey)
    if (!found) return null
    setEntry(agentId, { index: found.index })
    return { provider: found.provider, model: found.model, reasoningEffort: found.reasoningEffort, key: found.key }
  }
  // 有无可用备用（不推进游标），供 request-error 决策 retry vs 终态
  function hasFallback(agentId, excludeKey) { return findFallback(agentId, excludeKey) !== null }
  ...
  ctx.on('agent/request-error', async (payload, next) => {
    try {
      ...
      const failingKey = entry.lastKey && splitKey(entry.lastKey)[0] === provider
        ? entry.lastKey
        : keyOf(provider, '*')
      const maxRetries = Math.max(2, Number(cfg.threshold) + (cfg.backups || []).length)
  ...
  async function runOne(subagents, provider, task, extra, parent, signal, agentDef) {
    ...
    const prompt = (extra ? t('orch.mergedPrefix') + '\n\n' + extra + '\n\n---\n\n' : '') + task.prompt
    const request = {
    ...
    return defineTool({
      ...
      async execute(args, exec) {
        try {
          ...
          for (const tk of tasks) if (tk && tk.agent && !resolveAgentDef(tk.agent)) unknown.push(tk.agent)
      ...
      contextInjectDispose = sp.section({
        ...
        text: () => {
          ...
          if (orch && orch.enabled) {
            ...
            return t('orch.hintSection')
          }
          debugLog('debug', 'ctx.inject.eval', '上下文注入求值：无可用内容', { enabled: true, mode: 'empty' })
          return ''
        },
      })
      debugLog('info', 'ctx.inject.install', '上下文注入段落已注册', { section: 'ha-orchestrator:context', order: 500 })
    } catch (e) {
  ...
  remoteDecorate(HaOrchestratorRpc.prototype, 'stateGet', 'stateGet')
  remoteDecorate(HaOrchestratorRpc.prototype, 'stateReload', 'stateReload')
  remoteDecorate(HaOrchestratorRpc.prototype, 'stateSet', 'stateSet')
  remoteDecorate(HaOrchestratorRpc.prototype, 'modelsList', 'modelsList')
  remoteDecorate(HaOrchestratorRpc.prototype, 'agentsGenerate', 'agentsGenerate')
  remoteDecorate(HaOrchestratorRpc.prototype, 'haReset', 'haReset')
  remoteDecorate(HaOrchestratorRpc.prototype, 'debugLogs', 'debugLogs')
  remoteDecorate(HaOrchestratorRpc.prototype, 'debugClear', 'debugClear')
