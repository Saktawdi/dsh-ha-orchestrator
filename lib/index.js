...
async function apply(ctx) {
    ...
    let runSummaryCache = null;
    let runSummaryCacheAt = 0;
    function invalidateRunSummaryCache() {
    ...
    const systemicErrorsByRun = new Map();
    function recordEscapedRunError(runId, e) {
        if (!runId)
            return;
    ...
    function isSystemicRunError(runId, e) {
        if (!runId)
            return false;
    ...
    function summarizeRunRecord(rec) {
    ...
    async function readRunSummariesFromDisk() {
        if (runSummaryCache && now() - runSummaryCacheAt < RUN_SUMMARY_TTL_MS)
        ...
        try {
            ...
            for (const line of String(text).split(/\r?\n/)) {
                ...
                try {
                    const rec = JSON.parse(line);
                    if (rec && rec.runId)
                        out.push(summarizeRunRecord(rec));
                }
                catch (e) { /* 跳过损坏行 */ }
            }
    ...
    async function recentRunSummaries(limit, sessionIds) {
        const disk = await readRunSummariesFromDisk();
        const byId = new Map();
        for (const rec of state.runs)
            byId.set(rec.runId, summarizeRunRecord(rec));
        for (const rec of disk)
            if (!byId.has(rec.runId))
            ...
            async execute(args, exec) {
                ...
                try {
                    ...
                    if (resumePrevRecForCatch) {
                        const byId = new Map();
                        for (const r of resumePrevRecForCatch.runs.filter((r) => isUsableRunStatus(r.status)))
                            byId.set(r.id, r);
    ...
    async function runOrchestrateFromCommand(args, agent, signal) {
        const tool = orchestrateToolDef;
    ...
    function commandInput(invocation) {
    ...
    let resumeCommandDispose = null;
    let resumeCommandRetries = 0;
    const RESUME_COMMAND_MAX_RETRIES = 30;
    function installResumeCommand() {
        try {
            if (resumeCommandDispose) {
                resumeCommandDispose();
                resumeCommandDispose = null;
            }
        }
        catch (e) { /* ignore */ }
        const commands = getService(ctx, 'commands');
        ...
        try {
            resumeCommandDispose = commands.register({
                ...
                input: { hint: '<runId>' },
                handler: (invocation) => handleHaOrchResume(invocation),
            });
    ...
    function scheduleResumeCommandRetry() {
        if (resumeCommandRetries >= RESUME_COMMAND_MAX_RETRIES)
            return;
        ...
        if (!timer || typeof timer.timeout !== 'function')
            return;
        try {
            timer.timeout(() => installResumeCommand(), 2000);
        }
        catch (e) { /* ignore */ }
    }
    /**
     ...
     */
    function tryInjectResumeFollowup(agent, fromRunId, resumed) {
        try {
            const a = agent;
            ...
            const oc = state.config.orch;
            const rendered = renderRunOutput({ summary: String(resumed.summary || ''), runs: resumed.runs }, {
                runOutputLimit: Number(oc.renderRunLimit) > 0 ? Number(oc.renderRunLimit) : undefined,
                totalLimit: Number(oc.renderTotalLimit) > 0 ? Number(oc.renderTotalLimit) : undefined,
            });
            ...
            debugLog('error', 'orch.resume.followup', 'resume 结果注入失败', { message: String((e && e.message) || e) });
        }
    }
    async function handleHaOrchResume(invocation) {
        try {
            const parts = commandInput(invocation).trim().split(/\s+/).filter(Boolean);
            const runId = parts[0] || '';
            if (!runId)
                return { kind: 'error', text: t('orch.resumeUsage') };
            const agent = invocation.agent;
            if (!agent)
                return { kind: 'error', text: t('orch.errNoAgentCtx') };
            ...
            if (!prevRec)
                return { kind: 'error', text: t('orch.errNoRun', { runId }) };
    ...
    class HaOrchestratorRpc extends TypertRemoteService {
        ...
        async orchRecent(args = {}) {
            ...
            const sessionIds = Array.isArray(rawSessionIds)
                ? rawSessionIds.map((id) => String(id || '')).filter(Boolean).slice(0, 64)
                : [];
