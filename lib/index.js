        ...
        return defineTool({
            ...
            async execute(args, exec) {
                ...
                try {
                    ...
                    const worker = (task, i) => runOne(subagents, provider, task, '', parent, signal, defFor(task), runId);
                    ...
                    if (mode === 'pipeline') {
                        ...
                        for (let round = 1; round <= reviewRounds; round += 1) {
                            const roundPrompt = buildSupervisorPrompt(instruction, round === 1 ? merged : appendPipelineCarry(prevOut, merged), t('orch.outputSeparator'));
