...
export function sameTaskList(a, b) {
    if (!Array.isArray(a) || !Array.isArray(b) || a.length !== b.length)
        return false;
    for (let i = 0; i < a.length; i += 1) {
        if (taskSignature(a[i]) !== taskSignature(b[i]))
            return false;
    }
    return true;
}
...
export function buildSubagentRequest(task, extra, agentDef, mergedPrefix, parent, signal, defaultMaxTokens = 0) {
...
export function isUsableRunStatus(status) {
...
export function isFallbackEligibleError(e, signal) {
    ...
    if (err.isolate === false)
        return false;
    if (String(err.name || '') === 'AbortError')
        return false;
    const msg = String(err.message || e);
    // 本插件自产的固定错误串（非 i18n，稳定可匹配）：缺取消信号属请求构造缺陷
    if (msg.indexOf('runOne: 缺少取消信号') >= 0)
        return false;
    return true;
}
