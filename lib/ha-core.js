// ha-core.js -- DSH-agnostic pure HA (high-availability) state machine helpers.
...
export function findFallback(state, haCfg, registeredProviders, agentId, excludeKey, now = Date.now()) {
  ...
  const registered = registeredProviders ? (registeredProviders instanceof Set ? registeredProviders : new Set(registeredProviders)) : new Set()
